	...
	<c:if test="${menu.parentId eq parentId && menu.isShow eq '1' }">
		<div class="accordion-group">
			<div class="accordion-heading">
            	...
            	<i class="icon-chevron-${first?'down':'right' }"></i>&nbsp;${menu.name }</a>
       		</div>
       		<div id="collapse-${menu.id }" class="accordion-body ${first?'in':'' } collapse"<%--  style="height: ${first?'auto':0 };" --%>>
	            <div class="accordion-inner">
	                <ul class="nav nav-list">
	                    <c:forEach items="${menuList }" var="menu2">
	                    	<c:if test="${menu2.parentId eq menu.id && menu2.isShow eq '1' }">
		                    	<li><a data-href=".menu3-${menu2.id }" href="${ctx}${menu2.href}" target="mainFrame"><i class="icon-${menu2.icon }"></i>&nbsp;${menu2.name }</a>
